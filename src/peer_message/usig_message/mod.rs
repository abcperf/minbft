...
//! Such messages are either of inner type [ViewPeerMessage], [ViewChangeV], [NewView] or [Checkpoint].
...
/// A [UsigMessage] is a message that contains a USIG signature,
/// and is either a [ViewPeerMessage], [ViewChangeV], [NewView] or a [Checkpoint].
#[derive(Serialize, Deserialize, Clone, Debug)]
pub(crate) enum UsigMessageV<V: ViewChangeVariant<P, Sig>, P, Sig> {
    /// A [UsigMessage] of type [ViewPeerMessage].
    View(ViewPeerMessage<P, Sig>),
    /// A [UsigMessage] of type [ViewChangeV].
    ViewChange(ViewChangeV<V, P, Sig>),
    /// A [UsigMessage] of type [NewView].
    NewView(NewView<P, Sig>),
    /// A [UsigMessage] of type [Checkpoint].
...
{
    /// Create a [UsigMessage] based on a [ViewPeerMessage].
...
impl<V: ViewChangeVariant<P, Sig>, P, Sig> From<ViewChangeV<V, P, Sig>>
    for UsigMessageV<V, P, Sig>
{
    /// Create a [UsigMessage] based on a [ViewChangeV].
...
impl<V: ViewChangeVariant<P, Sig>, P, Sig> From<NewView<P, Sig>> for UsigMessageV<V, P, Sig> {
    /// Create a [UsigMessage] based on a [NewView].
    fn from(new_view: NewView<P, Sig>) -> Self {
        Self::NewView(new_view)
    }
}

/// A not further specified [UsigMessage] is a [UsigMessage] that contains a log of messages if it is of inner type [ViewChangeV].
...
impl<P: Clone + Serialize, Sig: Clone + Serialize> UsigMessage<P, Sig> {
    ...
    /// (only in case of a [ViewChangeV] message).
...
impl<V: ViewChangeVariant<P, Sig>, P, Sig: Counter> Counter for UsigMessageV<V, P, Sig> {
    /// Returns the counter of the [UsigMessage] by returning the counter of its inner type.
...
impl<V: ViewChangeVariant<P, Sig>, P, Sig> AsRef<ReplicaId> for UsigMessageV<V, P, Sig> {
    /// Referencing [UsigMessageV] returns a reference to the origin of the [UsigMessageV]
    /// (either of the [ViewPeerMessage], [ViewChangeV], [NewView] or [Checkpoint]).
